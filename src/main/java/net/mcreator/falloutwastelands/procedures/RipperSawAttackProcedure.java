...
public class RipperSawAttackProcedure {
		...
		if (entity.getPersistentData().getBoolean("RipperSaw") == true) {
				...
				if (!projectileLevel.isClientSide()) {
					Projectile _entityToSpawn = new Object() {
						public Projectile getArrow(Level level, Entity shooter, float damage, int knockback) {
							AbstractArrow entityToSpawn = new RipperProjectileProjectileEntity(FalloutWastelandsModEntities.RIPPER_PROJECTILE_PROJECTILE.get(), level);
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("fallout_wastelands_:ripper")), SoundSource.NEUTRAL, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("fallout_wastelands_:ripper")), SoundSource.NEUTRAL, 1, 1, false);
				}
			}
		}
	}
